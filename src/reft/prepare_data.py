...
def create_dpo_dataset(config):
    ...
    try:
        print(f"Step 1/4: Loading the original dataset '{config['data']['dataset_name']}'...")
        full_dataset = load_dataset(config['data']['dataset_name'], split="train")
        train_test_split = full_dataset.train_test_split(test_size=0.1, seed=config['data']['seed'])
        ...
        print(f"Step 2/4: Reading 'chosen' file from '{config['data']['judgments_path']}'...")
        judgments_df = pd.read_csv(config['data']['judgments_path'])
        ...
        print(f"Step 3/4: Reading 'rejected' file from '{config['data']['model_responses_path']}'...")
        model_responses_df = pd.read_csv(config['data']['model_responses_path'])
    ...
    output_path = config['data']['dpo_dataset_path']
